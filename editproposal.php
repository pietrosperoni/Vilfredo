...
{	
	...
	AND `proposals`.`usercreatorid` = $userid
	AND `questions`.`id` = `proposals`.`experimentid` 
	AND `questions`.`roundid` = `proposals`.`roundid` 
	AND `questions`.`phase` = 0";
	
	$response = mysql_query($sql);
	
	if (!$response || !mysql_num_rows($response))
	{
		set_log(__PAGE__.":: Failed to retrieve proposal $proposal");
		setError("For some reason we were unable to retrieve that proposal.");
		header("Location: error_page.php");
		exit;
	}

	if ($row = mysql_fetch_assoc($response))
	{		
		$blurb =	$row['blurb'];
		...
		<div id="actionbox">
			...
			<p>Remember to save your edited proposal.</p>
		
			<form method="POST" action="editproposaltake.php">			
     			<div id="editor_panel">
				...
				<div id="abstract_panel">
					...
					<div id="p_abstract_RTE">
						      ...
						      <?php
							$RTE_TextLimit_abstract = MAX_LEN_PROPOSAL_ABSTRACT;
...
try{
		...
		$("#abstract_rte").data('content_length', 0)
			     ...
			     <div id="proposal_RTE">
			      ...
			      <?php
			         $RTE_TextLimit_content = MAX_LEN_PROPOSAL_BLURB;
				...
				<input type="submit" name="submit" id="saveprop" value="Save Proposal" disabled="disabled"/>
				<input type="button" name="cancel" id="canceledit" value="Cancel"/>
				</div> <!-- proposal_RTE -->
				</div> <!-- editor_panel -->
...
function goTo(url)
{
	url = jQuery.type(url) === "string" ? url : '<?=SITE_DOMAIN?>';
